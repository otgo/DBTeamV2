...
local function run(msg, matches)
   if permissions(msg.from.id, msg.to.id, "gban") and redis:get("moderation_group: " .. msg.to.id) then
		if not matches[2] then
			local count = tonumber(redis:scard("gbans"))
			local text = gettext(ln.gbans.gbans, count)
		...
		elseif matches[2] == "install" then
			...
			for k, v in pairs(t) do
				if string.match(v, "^gbans%.[Ll]ua$") then
					...
					send_msg(msg.to.id, gettext(ln.gbans.gbanLua, count), "html")
				...
				elseif string.match(v, "^gbans%.[Jj][sS][Oo][nN]$") then
					...
					f:close()
					send_msg(msg.to.id, gettext(ln.gbans.gbanJson, count), "html")
				end
			end

		elseif matches[2] == "delete" then
			redis:del("gbans")
			send_msg(msg.to.id, gettext(ln.gbans.gbanDel), "html")
		end
   end
end
